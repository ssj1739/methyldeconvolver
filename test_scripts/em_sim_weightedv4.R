...
#write.csv(sim.out,"C:/Users/Patri/Documents/School/Georgetown/Thesis/Deconv Sim/sim.out.15.50.weighted.csv", row.names = FALSE)

p <- ggplot(data = sim.out, aes(x=cell_idx, y=alpha, group=cell_idx)) + 
  geom_boxplot(aes(fill=cell_idx),alpha=0.3) +
  labs(title="15 Cell Types - 50 Markers Per - Weighted Model",
...
summary <- sim.out %>% group_by(model,zeta1,cell_idx)%>%summarise(mean = mean(alpha), sd = sd(alpha))
rmse <- sim.out %>% group_by(model,zeta1)%>%summarise(rmse = mean(rmse))
